...
app="conv_layer_process"
run_dir="/work/rtohid/halide-tutorials/hpx/apps/conv_layer"
args=""

#threads=(1 2 4 8 12 16 20 24 28 32 36 40)
threads=(16)
hpx_dir=/work/rtohid/hpx/
halide_dir=/work/rtohid/Halide/
...
do
	for iter in {1..6}
	do
		HL_NUM_THREADS=${num_th} perf stat -e task-clock,cycles,instructions,cache-references,cache-misses ${halide_run_dir}/cmake-build/${app} ${args} \
...
do
	for iter in {1..6}
	do
		HL_NUM_THREADS=1 OMP_NUM_THREADS=1 perf stat -e task-clock,cycles,instructions,cache-references,cache-misses ${hpx_run_dir}/cmake-build/${app} ${args} \
						   --hpx:threads=${num_th} \
