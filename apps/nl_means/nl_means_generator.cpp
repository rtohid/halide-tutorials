...
public:
  ...
  void generate() {
    ...
    clamped_with_alpha(x, y, c) =
        mux(c, {clamped(x, y, 0), clamped(x, y, 1), clamped(x, y, 2), 1.0f});

    // Define a reduction domain for the search area
    RDom s_dom(-(search_area / 2), search_area, -(search_area / 2),
               search_area);

    // Compute the sum of the pixels in the search area
    Func non_local_means_sum("non_local_means_sum");
    non_local_means_sum(x, y, c) +=
        w(x, y, s_dom.x, s_dom.y) *
        clamped_with_alpha(x + s_dom.x, y + s_dom.y, c);

    non_local_means(x, y, c) =
        clamp(non_local_means_sum(x, y, c) / non_local_means_sum(x, y, 3), 0.0f,
              1.0f);
    ...
    non_local_means.compute_root()
        ...
        .parallel(ty)
        .vectorize(x, vec);
    blur_d_y.compute_at(non_local_means, tx).reorder(y, x).vectorize(x, vec);
    d.compute_at(non_local_means, tx).vectorize(x, vec);
    ...
    non_local_means_sum.update(0)
        ...
        .unroll(c)
        .vectorize(x, vec);
    blur_d.compute_at(non_local_means_sum, x).vectorize(x, vec);
  }
};
