...
Func blur_cols_transpose(Func input, Expr height, Expr alpha,
                         bool skip_schedule, Target target) {
  Func blur("blur");
...
public:
  ...
  void generate() {
    ...
    Func blury_T =
        blur_cols_transpose(input, height, alpha, auto_schedule, get_target());

    // Blur the columns again (the rows of the original).
    Func blur =
        blur_cols_transpose(blury_T, width, alpha, auto_schedule, get_target());
